        ...
        <div class="max-w-8xl mx-auto px-3 sm:px-6 lg:px-8 flex flex-col gap-4">
            <div class="flex justify-center items-center w-full">
                <div class="bg-white shadow-sm w-full md:w-5/6 lg:w-3/4 h-auto px-6 py-4 rounded-lg">
                    <div class="mb-5">
                        <h3 class="text-lg font-semibold">Ubah Rule</h3>
                        <p class="text-sm text-slate-500">Silakan isi semua informasi yang dibutuhkan</p>
                    </div>
                    <form action="{{ route('dead-reckoning.update', $deadReckoning->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="aksi" id="aksi" class="block mt-1 w-full rounded-xl bg-gray-100">
                                    ...
                                    @foreach ($aksi as $item)
                                        <option value="{{ $item->id }}" @selected($deadReckoning->drone_dataset_id == $item->id)>
                                            {{ $item->nama_kondisi }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="durasi" class="block mt-1 w-full rounded-xl bg-gray-100"
                                    type="number" name="durasi" :value="old('durasi', $deadReckoning->durasi)" required autofocus
                            ...
                            <div>
                                ...
                                <select name="satuan_waktu" id="satuan_waktu"
                                    ...
                                    <option value="menit" @selected($deadReckoning->satuan_waktu == 'menit')>Menit</option>
                                    <option value="detik" @selected($deadReckoning->satuan_waktu == 'detik')>Detik</option>
                                    <option value="milidetik" @selected($deadReckoning->satuan_waktu == 'milidetik')>Milidetik</option>
                                </select>
                                <x-input-error :messages="$errors->get('aksi')" class="mt-2" />
                            </div>
