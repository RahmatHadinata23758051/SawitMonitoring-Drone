                    ...
                    <table class="w-full align-middle border-slate-400 table mb-0 px-2" id="drone-dataset-table">
                        <thead>
                            <tr>
                                ...
                                <th class="dt-center" rowspan="2">Kode</th>
                                <th class="dt-center" rowspan="2">Nama Kondisi</th>
                                <th class="dt-center border-x border-r-[#b2b2b2]" colspan="3">Accelero</th>
                                <th class="dt-center border-r border-r-[#b2b2b2]" colspan="3">Gyro</th>
                                <th class="dt-center" rowspan="2">Aksi</th>
                            </tr>
                            <tr>
                                <th class="dt-center border border-[#b2b2b2]">X</th>
                                <th class="dt-center border border-[#b2b2b2]">Y</th>
                                <th class="dt-center border border-[#b2b2b2]">Z</th>
                                <th class="dt-center border border-[#b2b2b2]">X</th>
                                <th class="dt-center border border-[#b2b2b2]">Y</th>
                                <th class="dt-center border border-[#b2b2b2]">Z</th>
                            </tr>
                        ...
                        <tbody class="table-border-bottom-0" id="kebun-tbody">
                            @foreach ($dataset as $item)
                                <tr>
                                    ...
                                    <td>{{ (float) $item->accel_x }}</td>
                                    <td>{{ (float) $item->accel_y }}</td>
                                    <td>{{ (float) $item->accel_z }}</td>
                                    <td>{{ (float) $item->gyro_x }}</td>
                                    <td>{{ (float) $item->gyro_y }}</td>
                                    <td>{{ (float) $item->gyro_z }}</td>
                                    <td class="h-full">
                                        <div class="flex items-center justify-center gap-2 h-full">
                                            <a href="{{ route('drone-dataset.edit', $item->id) }}" class="h-100">
                                                <i class="fa fa-pen text-yellow-500"></i>
                                            </a>
                                            <form action="{{ route('drone-dataset.destroy', $item->id) }}" method="POST"
                                                class="delete-form" data-kode="{{ $item->kode_kondisi }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit">
