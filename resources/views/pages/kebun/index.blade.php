            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                        <i class="fa-solid fa-tree text-primary"></i> Data Kebun
                    </h1>
                    <p class="text-sm text-slate-500 mt-0.5">Manajemen data kebun sawit</p>
                </div>
                <a href="{{ route('kebun.create') }}"
                    ...
                    <i class="fa-solid fa-circle-plus"></i> Tambah Kebun
                </a>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-slate-100">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
                        <i class="fa-solid fa-seedling text-primary text-sm"></i>
                    </div>
                    <h3 class="font-bold text-slate-800">Daftar Kebun</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full align-middle table mb-0 text-sm" id="kebun-table">
                        <thead>
                            <tr class="text-xs uppercase tracking-widest text-slate-500 bg-slate-50 border-b border-slate-200">
                                ...
                                <th class="dt-center px-4 py-3 font-semibold">Nama Kebun</th>
                                <th class="dt-center px-4 py-3 font-semibold">Luas Kebun</th>
                                <th class="dt-center px-4 py-3 font-semibold">Jumlah Pohon</th>
                                <th class="dt-center px-4 py-3 font-semibold">Lokasi Lahan</th>
                        ...
                        <tbody class="divide-y divide-slate-100" id="kebun-tbody">
                            @foreach ($kebun as $item)
                                <tr class="hover:bg-slate-50/70 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span class="text-xs bg-emerald-50 text-emerald-700 px-2 py-1 rounded-lg font-medium">{{ $item->lahan->nama }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-center">
        ...
        <script>
            ...
            window.addEventListener('load', function() {
                $(document).ready(function() {
                    $('#kebun-table').DataTable({ responsive: true, ordering: false, dom: '<"dt-toolbar flex justify-between items-center px-4 py-2 mb-2"Bf>rtp', buttons: [{ extend: 'excel', text: 'Export Excel', title: 'Data Kebun', className: 'bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600', filename: () => `data_kebun_${timestamp()}`, exportOptions: { columns: [0,1,2,3,4] } }], columnDefs: [{ className: "dt-center", targets: "_all" }], language: { emptyTable: "Tidak ada data kebun", paginate: { previous: "<", next: ">" }, zeroRecords: "Data tidak ditemukan.", search: "" } });
                ...
                document.querySelectorAll('.delete-form').forEach(form => {
                    form.addEventListener('submit', function(e) {
                        e.preventDefault();
                        Swal.fire({ title: 'Konfirmasi', text: `Hapus kebun "${this.dataset.nama}"?`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Ya, Hapus!', cancelButtonText: 'Batal' }).then(r => { if (r.isConfirmed) this.submit(); });
                    });
                });
            });
        </script>
