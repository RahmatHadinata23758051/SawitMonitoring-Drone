    ...
    <x-slot name="header">
        <h2 class="leading-tight">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item breadcrumb-active">{{ __('Kebun') }}</li>
            </ol>
        </h2>
    </x-slot>
                    ...
                    <table class="w-full align-middle border-slate-400 table mb-0 px-2" id="drone-dataset-table">
                        <thead>
                            <tr>
                                ...
                                <th class="dt-center">Tinggi Pohon (m)</th>
                                <th class="dt-center">Jarak Antar Pohon Sejalur (m)</th>
                                <th class="dt-center">Jarak Antar Pohon Menyamping (m)</th>
                        ...
                        <tbody class="table-border-bottom-0" id="kebun-tbody">
                            @foreach ($dataset as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->tinggi_pohon }}</td>
                                    <td>{{ $item->interval_pohon_sejalur }}</td>
                                    <td>{{ $item->interval_pohon_menyamping }}</td>
                                    <td class="h-full">
                                        <div class="flex items-center justify-center gap-2 h-full">
                                            <a href="{{ route('kebun-dataset.edit', $item->id) }}" class="h-100">
                                                <i class="fa fa-pen text-yellow-500"></i>
                                            </a>
                                            <form action="{{ route('kebun-dataset.destroy', $item->id) }}" method="POST"
                                                class="delete-form" data-kebun="{{ $item->kebun->nama }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit">
            ...
            window.addEventListener('load', function() {
                ...
                document.querySelectorAll('.delete-form').forEach(form => {
                    form.addEventListener('submit', function(event) {
                        ...
                        const namaKebun = this.getAttribute('data-kebun');
                        Swal.fire({
                            title: 'Konfirmasi',
                            text: `Apakah Anda yakin ingin menghapus dataset milik kebun ${namaKebun}?`,
                            icon: 'warning',
