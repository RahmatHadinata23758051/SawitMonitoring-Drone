    ...
    <div class="pt-6 pb-12">
        <div class="max-w-full mx-auto px-6 lg:px-10 flex flex-col gap-6">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-black text-slate-800">Dataset Kebun</h1>
                    <p class="text-sm text-slate-500 mt-1">Data parameter fisik kebun untuk navigasi drone</p>
                </div>
                <div class="flex items-center gap-3 shrink-0">
                    <div class="relative">
                        <i class="fa-solid fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                        <input type="text" id="search-kdataset" placeholder="Cari dataset..."
                    ...
                    <a href="{{ route('kebun-dataset.create') }}"
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm" id="kdataset-table">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50/80">
                                ...
                                <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-widest">Tinggi Pohon (m)</th>
                                <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-widest">Jarak Sejalur (m)</th>
                                <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-widest">Jarak Samping (m)</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($dataset as $item)
                                <tr class="hover:bg-slate-50/60 transition-colors">
                                    <td class="px-5 py-4">
                                        <span class="font-mono font-bold text-primary text-sm">{{ $item->kebun->nama }}</span>
                                    </td>
                                    <td class="px-5 py-4 font-bold text-slate-800">{{ $item->jumlah_pohon }}</td>
                                    <td class="px-5 py-4 font-mono text-slate-700">{{ $item->tinggi_pohon }}</td>
                                    <td class="px-5 py-4 font-mono text-slate-700">{{ $item->interval_pohon_sejalur }}</td>
                                    <td class="px-5 py-4 font-mono text-slate-700">{{ $item->interval_pohon_menyamping }}</td>
                                    <td class="px-5 py-4">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('kebun-dataset.edit', $item->id) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-sky-500 text-white transition hover:bg-sky-600 shadow-sm">
                            ...
                            @empty
                                <tr><td colspan="6" class="px-5 py-16 text-center text-slate-400">
                                    <i class="fa-solid fa-table text-4xl opacity-20 block mb-3"></i>Belum ada dataset kebun
                                </td></tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    @push('scripts')
        <script>
            window.addEventListener('load', function() {
                document.getElementById('search-kdataset').addEventListener('input', function() {
                    const q = this.value.toLowerCase();
                    document.querySelectorAll('#kdataset-table tbody tr').forEach(row => { row.style.display = row.textContent.toLowerCase().includes(q) ? '' : 'none'; });
                });
